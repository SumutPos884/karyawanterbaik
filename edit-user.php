...
if(isset($_POST['submit'])):
	...
	$email = $_POST['email'];
	$role = $_POST['role'];
	$security_answer = $_POST['security_answer'];  // New security_answer input
	...
	if(empty($errors)):
		$update = mysqli_query($koneksi,"UPDATE user SET nama = '$nama', email = '$email', role = '$role', security_answer = '$security_answer' WHERE id_user = '$id_user'");
		
		if($password) {
			$pass = sha1($password);
			$update = mysqli_query($koneksi,"UPDATE user SET nama = '$nama', password = '$pass', email = '$email', role = '$role', security_answer = '$security_answer' WHERE id_user = '$id_user'");
		}		
		if($update) {
	...
	<div class="card shadow mb-4">
		...
		<div class="card-body">
			<div class="row">
				...
				<div class="form-group col-md-6">
					<label class="font-weight-bold">Jawaban Keamanan</label>
					<input autocomplete="off" type="text" name="security_answer" required value="<?php echo $d['security_answer']; ?>" class="form-control"/>
				</div>
			</div>
		</div>
